  ...
  const pages = useMemo<PaginationItemType[]>(() => {
    ...
    if (totalPageNumbers >= total) {
      ...
      if (!showLeftEllipsis && showRightEllipsis) {
        ...
        leftRange.forEach((pageNum) => items.push({ type: 'page', value: pageNum, isCurrent: pageNum === page }))
        ...
        rightRange.forEach((pageNum) => items.push({ type: 'page', value: pageNum, isCurrent: pageNum === page }))
      } else if (showLeftEllipsis && showRightEllipsis) {
        items.push({ type: 'page', value: 1, isCurrent: page === 1 })
        items.push({ type: 'ellipsis', key: leftSiblingIndex - 1 })
        const middleRange = range(leftSiblingIndex, rightSiblingIndex)
        middleRange.forEach((pageNum) => items.push({ type: 'page', value: pageNum, isCurrent: pageNum === page }))
...
const Trigger: FC<TriggerProps> = ({ children, style, className, asChild = false, direction, ariaLabel }) => {
  ...
  if (asChild && isValidElement(children)) {
    return cloneElement(children, {
      ...
      className:
        [computedClassName, (children.props as HTMLAttributes<HTMLElement>).className].filter(Boolean).join(' ') ||
        undefined,
